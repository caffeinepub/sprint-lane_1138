...
import type { BoardMember, Card, Tag, UserProfile } from "../../backend.d";
import { useDeleteCard, useUpdateCard } from "../../hooks/useQueries";
...
export const CardModal: React.FC<CardModalProps> = ({
    ...
    <>
      ...
      <div className="px-6 py-5 space-y-6 overflow-y-auto max-h-[60vh] md:max-h-none">
        ...
        <div>
          <label
            ...
            className="block text-sm font-medium text-sprint-text-secondary mb-2"
          >
            Description
          </label>
          <textarea
        ...
        {/* Tags */}
        <div>
          <p className="block text-sm font-medium text-sprint-text-secondary mb-2">
            Tags
          </p>
          {tags.length > 0 ? (
                ...
                >
                  <div
                    className={`
                      ...
                      ${isSelected ? "bg-sprint-accent-500 text-white" : "bg-sprint-accent-900/40 text-sprint-accent-400"}
                    `}
                  >
                  ...
                  {member.role === "owner" && (
                    <span className="text-[10px] bg-sprint-accent-900/40 text-sprint-accent-400 px-1.5 py-0.5 rounded">
                      Owner
                    </span>
                  )}
                </button>
      ...
      <div className="flex items-center justify-between px-6 py-4 border-t border-sprint-border bg-sprint-bg-secondary/30 rounded-b-xl ">
        <button
          ...
          className="flex items-center gap-2 px-3 py-2 text-sm text-red-500 hover:bg-red-900/20 rounded-lg transition-colors"
        >
          <svg
            aria-hidden="true"
            className="w-4 h-4"
      ...
      {showDeleteConfirm && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center rounded-xl z-10">
          <div className="bg-sprint-surface rounded-xl shadow-xl p-6 m-4 max-w-sm w-full">
    ...
    return (
      <Drawer.Root open={true} onOpenChange={(open) => !open && onClose()}>
        <Drawer.Portal>
          ...
          <Drawer.Content className="fixed bottom-0 left-0 right-0 z-50 bg-sprint-surface rounded-t-2xl outline-none max-h-[90vh] flex flex-col">
            {/* Drag handle */}
  ...
  return createPortal(
    <div
      ...
      role="presentation"
    >
      <div className="bg-sprint-surface rounded-xl shadow-xl max-w-2xl w-full my-8 relative">
        {ModalContent()}
      </div>
    </div>,
    document.body,
